...
class ShiftRepository extends \Doctrine\ORM\EntityRepository
{
    public function findPlanning($firstDate, $lastDate, $teamId)
    {
        ...
        $qb->select(array('s.id', 's.description', 's.startTime', 's.endTime', 's.date', 'u.id as userId', 'u.username', 'u.realName', 'tm.id as team_fk', 'st.id as shiftTypeId', 'st.short as shiftTypeShort', 'st.color as shiftColor', 't.id as taskId', 't.description as taskDescription', 't.startTime as taskStartTime', 't.endTime as taskEndTime'))
